...
public:
    ComputeTask(std::string name, int value, TaskPriority prio)
        : Task(std::move(name), prio)
        , value_(value)
    {}

    // 执行任务：模拟耗时计算（平方运算），并打印结果
    void execute() override {
        if (isCancelled()) return;  // 若任务已被取消则提前退出
...
int main() {
    ...
    scheduler.submit("low-1",      [] { std::printf("[FnTask] Low priority #1\n");      }, TaskPriority::Low);
    scheduler.submit("low-2",      [] { std::printf("[FnTask] Low priority #2\n");      }, TaskPriority::Low);
    scheduler.submit("normal-1",   [] { std::printf("[FnTask] Normal priority #1\n");   }, TaskPriority::Normal);
    scheduler.submit("normal-2",   [] { std::printf("[FnTask] Normal priority #2\n");   }, TaskPriority::Normal);
    scheduler.submit("high-1",     [] { std::printf("[FnTask] High priority #1\n");     }, TaskPriority::High);
    scheduler.submit("critical-1", [] { std::printf("[FnTask] Critical priority #1\n"); }, TaskPriority::Critical);
